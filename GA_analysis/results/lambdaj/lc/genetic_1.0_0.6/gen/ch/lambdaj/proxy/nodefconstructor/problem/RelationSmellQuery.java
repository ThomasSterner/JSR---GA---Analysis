...
public class RelationSmellQuery {
    ...
    public RelationSmell getSmellForID(final ElementId from, final ElementId to) {
        return selectFirst(smells, havingValue(on(RelationSmell.class).isForRelation(from, to)));
        /*
    ...
    public void when_selecting_first_element_for_relation_smell_query_should_not_throw() {
        RelationSmell smell_between_1_and_1 = new RelationSmell(new ElementId("1"), new ElementId("2"));
        RelationSmell smell_between_3_and_4 = new RelationSmell(new ElementId("3"), new ElementId("4"));
        smells = Arrays.asList(smell_between_1_and_1, smell_between_3_and_4, smell_between_1_and_1);
